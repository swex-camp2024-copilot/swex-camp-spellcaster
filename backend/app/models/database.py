...
class PlayerDB(SQLModel, table=True):
    ...
    sessions_as_player_1: List["SessionDB"] = Relationship(
        back_populates="player_1", sa_relationship_kwargs={"foreign_keys": "SessionDB.player_1_id"}
    )
    sessions_as_player_2: List["SessionDB"] = Relationship(
        back_populates="player_2", sa_relationship_kwargs={"foreign_keys": "SessionDB.player_2_id"}
    )
    game_results_as_winner: List["GameResultDB"] = Relationship(
        back_populates="winner", sa_relationship_kwargs={"foreign_keys": "GameResultDB.winner_id"}
    )
    game_results_as_loser: List["GameResultDB"] = Relationship(
        back_populates="loser", sa_relationship_kwargs={"foreign_keys": "GameResultDB.loser_id"}
    )
...
class SessionDB(SQLModel, table=True):
    ...
    player_1: PlayerDB = Relationship(
        back_populates="sessions_as_player_1", sa_relationship_kwargs={"foreign_keys": "SessionDB.player_1_id"}
    )
    player_2: PlayerDB = Relationship(
        back_populates="sessions_as_player_2", sa_relationship_kwargs={"foreign_keys": "SessionDB.player_2_id"}
    )
...
class GameResultDB(SQLModel, table=True):
    ...
    winner_id: Optional[str] = Field(
        foreign_key="players.player_id", default=None, description="Winner player ID (None for draw)"
    )
    loser_id: Optional[str] = Field(
        foreign_key="players.player_id", default=None, description="Loser player ID (None for draw)"
    )
    ...
    winner: Optional[PlayerDB] = Relationship(
        back_populates="game_results_as_winner", sa_relationship_kwargs={"foreign_keys": "GameResultDB.winner_id"}
    )
    loser: Optional[PlayerDB] = Relationship(
        back_populates="game_results_as_loser", sa_relationship_kwargs={"foreign_keys": "GameResultDB.loser_id"}
    )
