...
async def list_admin_players() -> List[dict]:
    ...
    infos = await runtime.admin_service.list_all_players()
...
async def list_active_sessions() -> List[dict]:
    ...
    infos = await runtime.admin_service.get_active_sessions()
    return [info.__dict__ for info in infos]
...
async def admin_cleanup_session(session_id: str) -> dict:
    ...
    ok = await runtime.admin_service.cleanup_session(session_id)
    if not ok:
