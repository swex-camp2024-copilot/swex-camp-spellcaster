...
class PlayerRegistrationError(PlaygroundError):
    ...
    def __init__(self, reason: str, **kwargs):
        super().__init__(
            f"Player registration failed: {reason}",
            status_code=400,
            **kwargs
        )
...
class BotTimeoutError(PlaygroundError):
    ...
    def __init__(self, timeout_seconds: float, session_id: Optional[str] = None, **kwargs):
        super().__init__(
            f"Bot execution timed out after {timeout_seconds} seconds",
            status_code=408,
            session_id=session_id,
            **kwargs
        )
