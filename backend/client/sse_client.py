"""Async SSE client for the Spellcasters Playground backend.

Features:
- Connects to backend SSE endpoint `/playground/{session_id}/events`
- Robust connection lifecycle with automatic reconnection (exponential backoff)
- SSE line parsing and event decoding into Pydantic models when available
- Clean async iterator API: `async for event in client.events(): ...`

Usage:
    async with SSEClient(base_url, session_id).connect() as client:
...
# These models are optional for decoding. Fallback to dict if import fails.
try:
    from backend.app.models.events import (
        Event as BackendEventUnion,
        TurnEvent,
        GameOverEvent,
        HeartbeatEvent,
        ErrorEvent,
...
except Exception:  # pragma: no cover - client can run outside backend package
...
class SSEClient:
    """Async SSE client with reconnection and event parsing."""
    ...
    async def connect(self) -> AsyncGenerator["SSEClient", None]:
        """Context manager that ensures client creation and cleanup."""
        if self._client is None:
            # httpx requires either a single default timeout or all four components
    ...
    async def events(self) -> AsyncIterator[Dict[str, Any]]:
        """Async iterator yielding parsed events (dict or Pydantic model dict).
        ...
        while not self._stop.is_set():
            try:
                ...
                # If we exit the stream normally, attempt reconnect unless stopped
    ...
    async def _iter_sse(self, resp: httpx.Response) -> AsyncIterator[Dict[str, str]]:
        """Iterate SSE events from response and yield dicts like {"event": ..., "data": ...}.

        Supports the subset of SSE used by our backend (event + data lines).
        """
    ...
    def _decode_event(self, sse: Dict[str, str]) -> Dict[str, Any]:
        """Decode SSE payload JSON into a typed model dict when available."""
        try:
            data = json.loads(sse.get("data", "{}"))
        except json.JSONDecodeError:
            return {"event": sse.get("event", "message"), "data": sse.get("data")}

        # If backend models are available, try to coerce based on `event` field
        if _HAVE_BACKEND_MODELS and isinstance(data, dict):
            ...
            if model is not None:
                ...
                except Exception:  # Fallback to raw dict if model validation fails
                    logger.debug("Failed to decode event with model; returning raw dict")
                    return data

        # Default: return parsed dict
        return data
